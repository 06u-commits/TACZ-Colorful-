...
public class ClientMessageTritiumColor {
    ...
    private ClientMessageTritiumColor() {
    }

    public ClientMessageTritiumColor(@NotNull ItemStack gun, int gunSlotIndex) {
        if (gun.getItem() instanceof IGun iGun) {
            for (AttachmentType type : AttachmentType.values()) {
                ItemStack attachment = iGun.getAttachment(gun, type);
                if (attachment.getItem() instanceof IAttachment iAttachment) {
                    if (iAttachment.hasCustomTritiumColor(attachment)) {
                        colorMap.put(type, iAttachment.getTritiumColor(attachment));
                    }
                }
            }
            if (iGun.hasCustomTritiumColor(gun)) {
                this.gunColor = iGun.getTritiumColor(gun);
    ...
    public static ClientMessageTritiumColor decode(FriendlyByteBuf buf) {
        ClientMessageTritiumColor message = new ClientMessageTritiumColor();
    ...
    public static void handle(ClientMessageTritiumColor message, Supplier<NetworkEvent.Context> contextSupplier) {
            ...
            context.enqueueWork(() -> {
                ...
                if (iGun != null) {
                    for (var entry : message.colorMap.entrySet()) {
                        ...
                        if (attachment.getItem() instanceof IAttachment iAttachment) {
                            iAttachment.setTritiumColor(attachment, color);
                        }
                    }
                    if (message.applyGunColor) {
                        iGun.setTritiumColor(gunItem, message.gunColor);
                    }
                }
            });
        }
